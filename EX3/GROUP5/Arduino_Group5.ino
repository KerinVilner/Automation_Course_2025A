...
void loop() {
  ...
  if (detectProximity(5)) {
    ...
    if (waitForButtonPress()) {
      if (buzzerActivated) {
        Serial.println("Button pressed. Opening door (activate servo).");
...
bool detectProximity(int targetDistance) {
  ...
  // תנאי להדפסת המרחק רק אם הוא קטן או שווה ל-5 ס"מ
